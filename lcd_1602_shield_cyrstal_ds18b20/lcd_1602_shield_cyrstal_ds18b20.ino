...
/*
Sketch uses 12,066 bytes (37.4%) of program storage space. Maximum is 32,256 bytes.
Global variables use 308 bytes (15.0%) of dynamic memory, leaving 1,740 bytes for local variables. Maximum is 2,048 bytes.
*/
...
void setup(){
    ...
    for (byte ds_num = 0; ds_num < ds_cnt; ds_num++){
        ...
        lcd->print(F("ИД"), 0, ds_num);
        lcd->print(addr_id[ds_num], -11, ds_num);
        lcd->print(F("\337C"), -1, ds_num);
    }
}
...
void loop(){
    ...
    for (byte ds_num = 0; ds_num < ds_cnt; ds_num++){
        ...
        lcd->print(celsius[ds_num], -3, ds_num, 4, 1);
    }
}
